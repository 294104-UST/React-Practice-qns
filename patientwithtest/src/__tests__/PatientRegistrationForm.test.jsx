...
describe('PatientRegistrationForm', () => {
  ...
  test('registers a new patient and clears the form data', async () => {
    const onRegisterMock = vi.fn(); // Mock the onRegister function
    const { getByPlaceholderText, getByText,getByDisplayValue } = render(<PatientRegistrationForm onRegister={onRegisterMock} />);
 
    // Fill out the form fields
    await userEvent.type(getByPlaceholderText('Name'), 'John Doe');
    await userEvent.type(getByPlaceholderText('Age'), '30');
    const genderCombobox = getByDisplayValue('Select Gender');
    await userEvent.selectOptions(genderCombobox, 'Male');
    await userEvent.type(getByPlaceholderText('Condition'), 'Hypertension');
    await userEvent.type(getByPlaceholderText('Last Visit (YYYY-MM-DD)'), '2022-01-15');
...
vi.mock('../PatientService'); // Mock the PatientService module
